    ...
    <v-row v-show="!isLoading.form">
      <v-col cols="3">
        <v-select
          ...
          :items="vagaStore.statusSolicitacao.sort(compararValor('statusIntermediario'))"
          density="compact"
    ...
    <v-table class="elevation-2" v-show="!isLoading.form">
      <thead>
        <tr>
          ...
          <th class="text-left text-grey">FUNÇÃO</th>
      ...
      <tbody>
        <tr v-for="item in vagas" :key="item.id">
          ...
          <td class="text-left">{{ item.funcaoNome }}</td>
...
async function showHistorico(item) {
  eventos.value = item.vagaHistorico;
...
async function getItems() {
  try {
    ...
    let response = await vagaStore.getPaginate(
      page.value,
      pageSize,
      filtros
    );
    vagas.value = response.items;
    totalPages.value = response.totalPages;
  } catch (error) {
    console.log("vagas.getItems.error:", error.response);
    handleErrors(error);
  } finally {

            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form @submit.prevent="salvarCentroCusto()" ref="centroCustoForm">
                        <v-row>
                            <v-col>
                                <v-text-field label="Código" v-model="centroCusto.codigo" type="number" required
...
import ClienteDadosBasicosForm from '@/components/share/clienteDadosBasicosForm.vue';
import {Cliente, useShareClienteStore } from '@/store/share/cliente.store';
...
async function salvar() {
    try {
        ...
        if (valid) {
            ...
            router.push({ name: "shareClientes" })
        }
...
async function salvarCentroCusto() {
    ...
    {
        let index = -1;
        if (centroCusto.value.id != 0)
        {
            index = cliente.value.centroCusto.findIndex(c => { return c.id == centroCusto.value.id })
            if (index > -1)
            {

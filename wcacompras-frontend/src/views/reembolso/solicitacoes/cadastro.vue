  ...
  try {
    ...
    if (parseInt(route.query.id) > 0) {
      ...
      let adiantamento = await solicitacaoStore.getByTipoAndUsuario(
        ...
        authStore.user.id,
        [1, 2, 3, 5]
      ); //removido o status 4 - rejeitado da verifição pedido por Luciano em 05/06/2024
...
async function baixarComprovantes() {
  try {
    ...
    console.error("baixarComprovantes", error)
    handleErrors(error, error.response.status == 404 ? "Não há comprovantes anexos!": "Houve um erro interno, favor contactar o administrador do sistema");
  } finally {

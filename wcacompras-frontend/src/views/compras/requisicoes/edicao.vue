        ...
        <v-form ref="formCadastro">
          <v-row>
            <v-col cols="5">
              <v-text-field
                label="Cliente"
                v-model="requisicao.cliente.nome"
                disabled
                density="compact"
                variant="outlined"
                color="primary"
            ...
            <v-col cols="5">
              <v-text-field
                label="Fornecedor"
                v-model="requisicao.fornecedor.text"
                disabled
                density="compact"
                variant="outlined"
                color="primary"
          ...
          <v-row>
            <v-col
              :cols="
                podeEditar && authStore.hasPermissao('requisicao_local_entrega')
                  ? 9
                  : 12
              "
            >
          ...
          <v-row>
            <v-col cols="4">
              <v-text-field
                label="Supervisor"
                v-model="requisicao.usuario.text"
                disabled
                density="compact"
                variant="outlined"
                color="primary"
            ...
            <v-col cols="3" class="text-center">
              <v-btn
                :color="getStatus(requisicao.status).color"
                variant="tonal"
                density="compact"
                class="text-center"
              >
                {{
                  ...
                  (requisicao.status == 0 ? aguardaQuem : "")
                }}
              </v-btn>
            </v-col>
            <v-col
          ...
          <v-form @submit.prevent="finalizarPedido()" ref="finalizarForm">
            ...
            <v-row>
              <v-col>
                <v-text-field
                  label="Data Entrega"
                  v-model="finalizarData.dataEntrega"
                  type="datetime-local"
                  required
                  variant="outlined"
                  color="primary"
...
const aguardaQuem = computed(() => {
  let texto = "";
  if (
    requisicao.value.requerAutorizacaoCliente &&
    requisicao.value.requerAutorizacaoWCA
  )
    texto = ": WCA / Cliente";
  else if (requisicao.value.requerAutorizacaoCliente) texto = ": Cliente";
  else if (requisicao.value.requerAutorizacaoWCA) texto = ": WCA";
...
const headerButtons = computed(() => {
  ...
  if (requisicao.value.status == 1 && authStore.hasPermissao("requisicao"))
    //aprovado
    buttons.push({
      text: "Solicitar Aprovação - Fornecedor",
      icon: "mdi-email-arrow-right-outline",
      event: "EmailFornecedorClick",
    });
  if (requisicao.value.requerAutorizacaoCliente && requisicao.value.status == 0)
    buttons.push({
      text: "Solicitar Aprovação - Cliente",
      icon: "mdi-email-arrow-right-outline",
      event: "EmailClienteClick",
    });
...
const hasChanged = computed(() => {
  let itensOriginals = JSON.stringify(requisicao.value.requisicaoItens)
    .replaceAll('"', "")
    .replaceAll(".00", "");
  if (requisicaoOriginal.value != null)
    itensOriginals = JSON.stringify(requisicaoOriginal.value.requisicaoItens)
      .replaceAll('"', "")
      .replaceAll(".00", "");

  let itens = JSON.stringify(requisicao.value.requisicaoItens)
    .replaceAll('"', "")
    .replaceAll(".00", "");

...
namespace wca.reembolso.application.Features.Faturamentos.Queries
{
    ...
    public record FaturamentoExportToExcelQuery(DateTime? DataIni, DateTime? DataFim, int FilialId =0, int ClienteId = 0, int Status = 0) : PaginationQuery, IRequest<ErrorOr<Stream>>;
    public sealed class FaturamentoExportToExcelQueryHandle : IRequestHandler<FaturamentoExportToExcelQuery, ErrorOr<Stream>>
    {
        ...
        public async Task<ErrorOr<Stream>> Handle(FaturamentoExportToExcelQuery request, CancellationToken cancellationToken)
        {
            ...
            sheet.Cell($"E{row}").SetValue("Status");
            sheet.Cell($"F{row}").SetValue("Valor");
            sheet.Cell($"G{row}").SetValue("P.O.");
            sheet.Cell($"H{row}").SetValue("Nota Fiscal");
            ...
            sheetDespesas.Cell($"E{row}").SetValue("Status");
            sheetDespesas.Cell($"F{row}").SetValue("Valor");
            sheetDespesas.Cell($"G{row}").SetValue("P.O.");
            sheetDespesas.Cell($"H{row}").SetValue("Nota Fiscal");
            ...
            foreach (var item in dados)
            {
                ...
                sheet.Cell($"E{row}").SetValue(item.Status switch
                {
                    1 => "AGUARDANDO P.O.",
                    2 => "P.O. EMITIDO",
                    3 => "FINALIZADO",
                ...
                sheet.Cell($"F{row}").SetValue(item.Valor).Style.NumberFormat.Format = numberFormat;
                sheet.Cell($"G{row}").SetValue(item.NumeroPO);
                sheet.Cell($"H{row}").SetValue(item.NotaFiscal);
                ...
                foreach (var faturamentoItem in item.FaturamentoItem)
                {
                    ...
                    sheetDespesas.Cell($"F{rowDespesa}").SetValue(sheet.Cell($"F{row}").Value).Style.NumberFormat.Format = numberFormat;
                    sheetDespesas.Cell($"G{rowDespesa}").SetValue(sheet.Cell($"G{row}").Value);
            ...
            sheet.Range("B2:H2").Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
            sheet.Range("B2:H2").Style.Fill.BackgroundColor = XLColor.BlueGray;
            sheet.Range("B2:H2").Style.Font.Bold = true;

            sheetDespesas.Range("B2:M2").Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
            sheetDespesas.Range("B2:M2").Style.Fill.BackgroundColor = XLColor.BlueGray;
            sheetDespesas.Range("B2:M2").Style.Font.Bold = true;
